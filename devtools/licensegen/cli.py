...
def generate(type: str, author: str, year: int, output: str, multi: bool):
    ...
    try:
        ...
        if multi:
            ...
            while True:
                try:
                    selection = click.prompt("Enter license numbers (e.g., 1,3,5)").split(",")
                    ...
                    if any(i < 1 or i > len(licenses) for i in selected_indices):
                        console.print("[red]Invalid selection. Please enter valid numbers.")
                        continue
        ...
        for license in selected_licenses:
            ...
            try:
                ...
                console.print(f"[green]✓[/green] Generated {license_type} license at: {output_path}")
            except ValueError as e:
                console.print(f"[red]Error generating {license_type} license:[/red] {str(e)}")
                continue
...
def format_permissions(permissions: list) -> str:
    ...
    readable_permissions = [
        permission_map.get(perm, perm.replace("-", " ").title())
...
def show_licenses():
    ...
    try:
        ...
        console.print("To generate multiple licenses, use: [bold]devtools license generate --multi[/bold]")
