...
@cli.command()
@click.option('--files', '-f', multiple=True, help='Specific files to generate commit messages for')
...
@click.option('--conventional/--no-conventional', default=True, help='Use conventional commit format')
...
def generate(files: tuple, repo: str, commit: bool, push: bool, conventional: bool, no_stage: bool, sign: bool, temperature: float):
    ...
    try:
        with Progress(
            ...
            commit_message = ai_service.generate_commit_message(staged_changes, temperature)
            progress.update(task, completed=True)
            ...
            if commit:
                if Prompt.ask("\nApply this commit message?", choices=["y", "n"], default="y") == "y":
                ...
                console.print("[yellow]Use --commit to apply the commit message.[/yellow]")
...
@click.option('--temperature', type=float, help='AI temperature (0.0-1.0)')
def generate_log(version: str, from_tag: str, days: int, commits: int, output: str, temperature: float):
    ...
    try:
        with Progress(
            ...
            if not entries:
                console.print("[yellow]No conventional commits found in the specified range.[/yellow]")
                return
            ...
            console.print(f"[green]Changelog generated successfully at {output_path}![/green]")
